...
the offsets exaggerated for legibility. The space switch is carried by the symbols
alone: (a) and (b) label everything in the v family, (c) labels its band in theta.
"""
...
def _panel_long_run(ax, rmax):
    ...
    setup(ax, "long run ($\\theta$-space)", rmax)
    ...
    # fill and edges drawn separately: a stroked annulus path shows its closing seam
    ax.add_patch(PathPatch(MplPath(np.vstack([band_outer, band_inner])),
                           facecolor=C_DISC, edgecolor="none", zorder=1))
    circle(ax, r=BAND_IN, color=C_RING, lw=1.0, z=1)
    circle(ax, r=BAND_OUT, color=C_RING, lw=1.0, z=1)

    circle(ax, r=RT_SHELL, color=C_RT, lw=3.6, z=5)
    circle(ax, r=HMC_SHELL, color=C_HMC, lw=2.8, z=4)

    # mirrored partner of the RT label: the leader ends in the band's outer
    # sliver, short of the blue ring, so it binds to the band and not to RT
    ax.annotate("typical set", xy=polar(1.095, 222.0), xytext=polar(1.70, 222.0),
                textcoords="data", fontsize=RING, color=C_LABEL,
                ha="right", va="center",
                arrowprops=dict(arrowstyle="-", color=C_LABEL, lw=0.9,
                                shrinkA=4, shrinkB=2))
    ax.annotate("HMC", xy=polar(HMC_SHELL, 35.0), xytext=(14, 9),
                textcoords="offset points", fontsize=METHOD, fontweight="bold",
                color=C_HMC, ha="left", va="center")
    ax.annotate("$O(\\sigma^2 h)$", xy=polar(HMC_SHELL, 35.0),
                xytext=(14, -9), textcoords="offset points", fontsize=RING,
                color=C_HMC, ha="left", va="center")
    # the RT label sits past the outer ring, so a thin line binds it back to
    # the blue shell
    ...
    ax.annotate("RT", xy=polar(RT_SHELL, -42.0), xytext=rt_anchor,
                ...
                arrowprops=dict(arrowstyle="-", color=C_RT_TEXT, lw=0.9,
                                shrinkA=4, shrinkB=2))
    ax.text(rt_anchor[0], rt_anchor[1] - 0.23, "$O(\\sigma^2 h^2)$",
