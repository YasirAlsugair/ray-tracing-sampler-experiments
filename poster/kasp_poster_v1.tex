% KASP 2026 ceremony poster v1, adapted from the SUDS v9 board, August 22, 2026
% A0 landscape (118.9 cm x 84.1 cm), beamerposter.
% Build:  cd empirical/poster && pdflatex kasp_poster_v1.tex
% Figures: pinned in artifacts/figures_v9/ (same verified set as SUDS v9).

\newcommand{\AuthorOne}{Yasir Alsugair}

\documentclass[final]{beamer}
\usepackage[size=custom,width=118.9,height=84.1,scale=1.22]{beamerposter}
...
\graphicspath{{artifacts/figures_v9/}{artifacts/figures_v8/}{artifacts/figures_v7/}{artifacts/figures_v6/}{artifacts/figures/}{figures/}}

\setbeamertemplate{navigation symbols}{}
\setbeamersize{text margin left=2.5cm, text margin right=2.5cm}

% KASP: KAUST Academy navy from kaustacademy.cls (SUDS board used 002A5C)
\definecolor{uoftblue}{HTML}{133C6A}
...
\newlength{\bandheight}\setlength{\bandheight}{11.7cm}
...
% name baseline so the marks sit comfortably within the header band
...
% Prefer the pinned v9 asset, then fall back to the locked v8 and earlier sets,
% and show a labeled placeholder rather than a failed build.
\newcommand{\figslot}[3]{% file, width fraction, label
  \IfFileExists{artifacts/figures_v9/#1}%
    {\includegraphics[width=#2\linewidth]{artifacts/figures_v9/#1}}%
...
\newcommand{\mcmcsectionbody}{%
  ...
  \vspace{-0.32cm}
  \begin{center}
    \figslot{minibatch_gradient.pdf}{0.88}{minibatch gradients against the exact one}
  \end{center}
  \vspace{-0.35cm}}
...
\newcommand{\noisegeometryscale}{0.88}
\newcommand{\noisegeometrypre}{\vspace{-0.42cm}}
\newcommand{\noisefigurewidth}{0.84}
\newcommand{\noisecodewidth}{0.14}
\newcommand{\noiseqrheight}{4.02cm}
% v7.1: the block opens by introducing the two movers on one clean landscape,
% so beat 1's "speed inflation vs directional change" lands on known nouns. Same
% start and tilt; HMC's launch speed is matched to RT's measured climb, so both
% paths reach the same target-density level before returning. That construction
% is deliberately left out of the visible annotation because it is not part of
% the gradient-noise argument.
\newcommand{\noiseintrowidth}{0.58}
\newcommand{\noiseblockopening}{%
  Hamiltonian Monte Carlo (HMC)\refmark{1} and ray tracing (RT)\refmark{2}
  both use the log-posterior gradient to build trajectories. HMC treats it as a
  force, changing a particle's speed and direction. RT treats the posterior as
  a refractive medium; its ray moves at unit speed, so the gradient changes only
  direction.

  \vspace{-0.22cm}
  \begin{center}
    \figslot{intro_dynamics.pdf}{\noiseintrowidth}{two movers, one landscape}
  \end{center}
  \vspace{-0.20cm}

  \textbf{1.\ One noisy step}

  Without a Metropolis correction, mean-zero noise raises HMC's expected
  kinetic energy and inflates its speed. For RT, with isotropic error
  $\sigma\zeta$ and $n(\theta)=\pi(\theta)^{1/(D-1)}$, the unit direction
  obeys
  \vspace{-0.03cm}
  \[
      \frac{\dd u}{\dd s}
  ...
  \resizebox{0.985\linewidth}{!}{$P_u$ projects onto the tangent plane, so noise
  changes $u$'s direction but not its norm.}
  \vspace{0.4cm}
}
...
% ---------------- header band ----------------
% top margin tightened for the A0 height budget
\vspace{1.15cm}
\headerband{%
  \rlap{%
    \vlogo{\logoslot{dept_logo_trim.png}{U of T Statistical Sciences signature}{3.6cm}{12cm}}%
    \hspace{1.6cm}%
    \vlogo{\logoslot{dsi_logo_standalone.png}{Data Sciences Institute logo}{2.6cm}{10cm}}}%
  \parbox[c]{\bandwidth}{\centering
    {\textcolor{uoftblue}{\textbf{\huge Toward Scalable Bayesian Uncertainty for Machine Learning\\[0.3ex] with the Ray Tracing Sampler}}}\\[0.40cm]
    {\LARGE \AuthorOne{}}\qquad
    {\large\textcolor{softgray}{Mentor:}\ Prof.\ Joshua Speagle
      \quad \textcolor{softgray}{Co-supervisor:}\ Prof.\ Ricardo Baptista}\\[0.22cm]
    {\large\textcolor{softgray}{KAUST Academy Summer School (KASP) 2026, hosted at the University of Toronto Data Sciences Institute (SUDS)}}\\[0.35cm]
    \vlogo{\logoslot{alagil_logo.png}{Alagil Foundation}{3.0cm}{8cm}}\hspace{2.2cm}%
    \vlogo{\logoslot{rzm_logo.png}{RZM}{3.2cm}{6cm}}\hspace{2.2cm}%
    \vlogo{\logoslot{moe_logo.pdf}{Saudi Ministry of Education}{3.3cm}{8cm}}}%
  \llap{%
    \vlogo{\logoslot{kaust_logo.png}{KAUST logo}{3.4cm}{10cm}}%
    \hspace{1.8cm}%
    \vlogo{\logoslot{kaust_academy_logo_trim.png}{KAUST Academy logo}{5.2cm}{12cm}}}%
}
\vspace{0.22cm}
\bandrule
\vspace{0.18cm}
...
\begin{center}
\figslot{data_bands.pdf}{0.52}{posterior draws vs one Adam fit}
\end{center}
\vspace{-0.3cm}
\textbf{Ordinary training:} one set of weights $\rightarrow$ one function.\\
\textbf{Bayesian sampling:} a distribution over weights $\theta$ $\rightarrow$
...
\vspace{0.25cm}
\begin{center}
\vlogo{\qrcode[height=2.3cm,level=M]{https://github.com/YasirAlsugair/ray-tracing-sampler-experiments}}%
\hspace{0.7cm}\parbox[c]{14.5cm}{\small Code and experiments:\\
  {\footnotesize\texttt{github.com/YasirAlsugair/}\\ \texttt{ray-tracing-sampler-experiments}}}
\end{center}
...
accumulate as \(\tfrac{S}{h}\cdot O(\sigma^2h^2) = O(\sigma^2h)\).
Let \(\sigma_c(h)\propto h^{-p}\) be the largest noise scale keeping long-run
bias below a fixed tolerance. Unadjusted minibatch HMC has \(p=\tfrac12\).
...
Across \(D=16\)--\(1024\): \(\sigma_c\propto h^{-1}\) (RT) versus
\(h^{-1/2}\) (HMC); real minibatch noise weakens RT's scaling.

\begin{center}
\makebox[\linewidth][c]{%
  \figslot{tolerance_erosion_frontier.pdf}{0.87}{tolerance, erosion and sampling cost}}
\end{center}
\end{block}
...
\figslot{gaia_calibration_by_group.pdf}{0.74}{per-group calibration, z std per label-error bin}\\[-0.74cm]
{\footnotesize z std = actual / claimed error: closer to 1 is better.}\\[0.48cm]
{\footnotesize averaging the draws beats the single best fit:}\\[0.04cm]
\figslot{exp7_cloud_poster.pdf}{0.72}{one star through 50 posterior draws: tight cloud, trusted prediction}\\[0.06cm]
\figslot{exp7_marginal_poster.pdf}{0.66}{the same two stars in \(y\): MAP spike vs marginalized mass}
\end{center}
\end{block}

\begin{block}{Conclusion}
\begin{itemize}\setlength{\itemsep}{0.02cm}
  \item The ray moves at a fixed speed, so a noisy gradient can only
    change its direction. This is why the sampler is robust to noise.
  \item We took the ray tracing sampler from its paper\refmark{2} to a
    ...
    guessing.
  \item The gains held on real data: on average, held-out stars were
    27\% more probable under the posterior average than under the single
    best fit, most of it from the averaging alone.
\end{itemize}
\end{block}
...
\noindent\textbf{Acknowledgments}\ \ The SUDS showcase poster behind this work
was joint work with Chuxuan Ai. We thank Peter Behroozi for the ray tracing
sampler and its released reference code. Supported by the Data Sciences
Institute, University of Toronto, and the KAUST Academy, with sponsor support
from the Alagil Foundation, RZM, and the Saudi Ministry of Education.
}
