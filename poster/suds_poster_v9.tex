% SUDS Showcase poster v9, August 14, 2026
% 4 ft x 3 ft landscape (121.92 cm x 91.44 cm), beamerposter.
% Build:  cd poster && pdflatex suds_poster_v9.tex
% Figures: every figure used by v9 is pinned in artifacts/figures_v9/.
...
\begin{center}
\makebox[\linewidth][c]{%
  \figslot{tolerance_erosion_frontier.pdf}{0.94}{tolerance, erosion and sampling cost}}
\end{center}
\end{block}
...
\begin{itemize}\setlength{\itemsep}{0.02cm}
  \item Ray tracing moves at fixed speed, so gradient noise changes the ray's
    direction, not its speed. Under an idealized isotropic model of this noise,
    halving the step size doubles the tolerated noise level. Since minibatch
    noise scales as \(B^{-1/2}\), this would allow the batch to be four times
    smaller at the same bias tolerance.
  \item Real minibatch noise departs from this model, weakening
    the ideal \(h^{-1}\) scaling. Even so, smaller steps tolerate more noise,
    and smaller batches reduce RT's cost per effective sample.
  \item On real stellar data, ray tracing provides well-calibrated uncertainty
    estimates and competitive predictive performance. Averaging 50 draws gives 27\%
    higher predictive probability on held-out stars than the best single fit.
\end{itemize}
\end{block}
