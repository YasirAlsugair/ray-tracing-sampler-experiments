...
Data\refmark{4,5}: 126{,}156 giants. Input: the star's Gaia XP spectrum
(110 coefficients). Output: its alpha abundance plus a per-star scatter,
with APOGEE values as the labels. The network (\(D = 11{,}394\)) is sampled
with minibatch ray tracing.
...
\begin{itemize}\setlength{\itemsep}{0.02cm}
  ...
  \item Real data is messier than the theory, but the advantage survives
    where it matters: the chains stay usable and cost less than the
    alternatives.
  \item This project took the sampler from its paper to a real survey
    posterior. The draws are the product: averaging them gives better
    predictions than one fitted network, and their spread warns us when
