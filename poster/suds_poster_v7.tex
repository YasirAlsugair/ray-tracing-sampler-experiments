...
\begin{itemize}\setlength{\itemsep}{0.02cm}
  \item Gradient noise can steer a constant-speed ray, but it cannot heat
    it, so the sampler is robust to noise. That one geometric fact is what
    makes Bayesian sampling with minibatches workable.
  \item Real data is messier than the theory, but the advantage survives
    ...
    alternatives.
  \item A sampled posterior gives what a single fitted network cannot:
    honest error bars, and a warning whenever the model itself is unsure.
\end{itemize}
\end{block}
