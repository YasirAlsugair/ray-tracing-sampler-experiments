...
\textbf{Result: the sampled error bars are correct, and averaging the
draws beats the single best fit.}

Data\refmark{4,5}: 126{,}156 giants; a network (\(D = 11{,}394\)) sampled
with minibatch ray tracing.\\
Input: the star's Gaia XP spectrum (110 coefficients).\\
Output: its alpha abundance plus a per-star scatter (labels from APOGEE).\\
