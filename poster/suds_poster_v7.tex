% SUDS Showcase poster v7, August 14, 2026
% 4 ft x 3 ft landscape (121.92 cm x 91.44 cm), beamerposter.
% Build:  cd poster && pdflatex suds_poster_v7.tex
% Figures: v7 overrides the updated Gaia panels in artifacts/figures_v7/;
%          unchanged figures fall back to the locked v6 and earlier figure sets.
...
\graphicspath{{artifacts/figures_v7/}{artifacts/figures_v6/}{artifacts/figures/}{figures/}}
...
% Prefer a v7 override, then fall back to the locked v6 and earlier asset sets,
...
Across \(D=16\)--\(1024\), idealized ray tracing fits \(p=1.00\); real
minibatches lower the local exponent.
...
and averaging its 50 draws makes each held-out star 27\% more probable than
the best single fit (MAP).}
Data\refmark{4,5}: 126{,}156 giants, each a 110-coefficient Gaia XP spectrum
with an APOGEE alpha label; a network (\(D = 11{,}394\)) maps spectrum to
abundance and scatter, sampled with minibatch ray tracing.
Key: \textbf{\textcolor{gaiamap}{point (MAP)}},
...
\begin{itemize}\setlength{\itemsep}{0.02cm}
  \item Noise cannot heat a ray that moves at constant speed, it can only
    turn it. That is why the tolerance exponent stays at \(p=1\) at every
    dimension we tested (\(D=16\) to \(1024\)), while minibatch HMC sits at
    \(p=\tfrac12\).
  \item Real minibatches do break this clean picture and the exponent
    drops. Even so, the sampler keeps working and costs less: about
    \(3.3\times\) less compute per effective sample, while minibatch HMC
    gets \(5.1\times\) more expensive. Whether \(p=1\) holds exactly in
    stationarity is still open.
  \item On the Gaia data the method held up: the error bars come out right
    in every group of stars, and averaging the draws makes each held-out
    star 27\% more probable than the single best fit. When the draws
    disagree about a star, that spread is a warning a single fit cannot
    give.
\end{itemize}
\end{block}
