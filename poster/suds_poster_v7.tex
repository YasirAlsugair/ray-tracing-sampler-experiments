...
\begin{itemize}\setlength{\itemsep}{0.02cm}
  \item The mechanism is the fixed speed. A noisy gradient can turn the
    ray, and turning is all it can do. That is where the robustness to
    noise comes from.
  \item With real minibatches the clean exponent erodes. The chains stay
    usable anyway, and an effective sample still costs less than with
    minibatch HMC.
  \item On the Gaia posterior, averaged draws beat the single fit and
    calibration held. When the draws scattered on a star, that spread told
    us not to trust the prediction there.
\end{itemize}
\end{block}
