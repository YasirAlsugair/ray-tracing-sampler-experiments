...
\begin{itemize}\setlength{\itemsep}{0.02cm}
  \item The mechanism is robust to gradient noise by construction: noise can
    only rotate the constant-speed ray, never heat it, and the measured
    tolerance exponent is \(p=1\) at every dimension tested
    (\(D=16\) to \(1024\)), against \(p=\tfrac12\) for minibatch HMC.
  \item On real data the clean mechanism breaks: minibatches erode the
    exponent. Performance stays reasonable and sampling gets cheaper
    (\(3.3\times\) less compute per effective sample; minibatch HMC
    \(5.1\times\) dearer). Open: is \(p=1\) exact in stationarity?
  \item Deployed on the Gaia posterior: per-group calibration
    stays flat, averaging the draws makes each held-out star 27\% more
    probable than the single best fit, and the posterior spread flags the
    stars where one fit is silently wrong.
\end{itemize}
\end{block}
